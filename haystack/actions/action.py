...
from haystack.actions._utils import DEFAULT_EDGE_NAME, ActionError, ActionValidationError, relevant_arguments
...
def haystack_action(callable):
    ...
    if inspect.isclass(callable):
        if not hasattr(callable, "run"):
            raise ActionError("Haystack class actions must have a run() method. See the docs for more information.")
