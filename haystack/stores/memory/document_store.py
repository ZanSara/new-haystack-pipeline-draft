...
from haystack.stores.memory._embedding_retrieval import get_scores_numpy, get_scores_torch, scale_to_unit_interval
...
class MemoryDocumentStore:
    ...
    def delete_index(self, index: str = "documents", delete_populated_index: bool = False) -> None:
        """
        ...
        """
        self.document_store.delete_index(index=index, delete_populated_index=delete_populated_index)
        if self.bm25:
            del self.bm25[index]
    ...
    def get_document_ids(self, filters: Dict[str, Any], index: str = "documents") -> Iterable[str]:
        """
    ...
    def get_documents(self, filters: Dict[str, Any], index: str = "documents") -> Iterable[Document]:
        """
    ...
    def write_documents(
        ...
        :return: None
        """
        self.document_store.write_items(items=(doc.to_dict() for doc in documents), index=index, duplicates=duplicates)
        if self.bm25 and len(documents) > 0:
            self.bm25[index].update_bm25(self.get_documents(filters={}, index=index))

    def delete_documents(self, ids: List[str], index: str = "documents", fail_on_missing_item: bool = False) -> None:
        """
        ...
        """
        self.document_store.delete_items(ids=ids, index=index, fail_on_missing_item=fail_on_missing_item)
    ...
    def get_relevant_documents(
        ...
        if not isinstance(queries[0], np.ndarray):
            raise ValueError("To use embedding retrieval, provide only the embeddings for the queries.")
    ...
    def _bm25_retrieval(
        ...
        if index not in self.bm25.keys():
            raise BM25RepresentationMissing(f"No BM25 representation for index {index}")   # TODO add a way to create such index

        filtered_document_ids = self.get_document_ids(filters={**filters, "content_type": "text"}, index=index),
    ...
    def _embedding_retrieval(
        ...
        for query in queries:
            ...
            filtered_documents = self.document_store.get_items(index=index, filters=filters)
            try:
                ids, embeddings = zip(*[(doc["id"], doc["embedding"]) for doc in filtered_documents])
            except KeyError:
                raise MissingEmbeddingError("Some of the documents don't have embeddings. Use the Embedder to compute them.")

            # At this stage the iterable gets consumed.
            if self.device.type == "cuda":
                scores = get_scores_torch(query=query, documents=embeddings, similarity=similarity, batch_size=batch_size, device=self.device)
            else:
                scores = get_scores_numpy(query, filtered_documents, similarity=similarity)

            top_k_ids = list(zip(ids, scores)).sort(key = lambda x: x[1] if x[1] is not None else 0.0, reverse=True)[:top_k]
