...
from haystack.stores._utils import StoreError, DuplicateError, IndexFullError, MissingItemError
...
class MemoryStore:
    ...
    def delete_index(self, index: str, delete_populated_index: bool = False) -> None:
        ...
        if items_count > 0:
            ...
            logger.warning(f"You are deleting index '{index}' which contains {items_count} items.")
    ...
    def has_item(self, id: str, index: str) -> bool:
        ...
        try:
            ...
            raise StoreError(f"No index names {index}. Create it with .create_index()") from e
    ...
    def get_items(self, filters: Dict[str, Any], index: str) -> Iterable[Dict[str, Any]]:
        """
    ...
    def delete_items(self, ids: List[str], index: str, fail_on_missing_item: bool = False) -> None:
        """
        ...
        for id in ids:
            if not self.has_item(id=id, index=index):
                if fail_on_missing_item:
                    raise MissingItemError(f"ID {id} not found in index {index}, cannot delete it.")
