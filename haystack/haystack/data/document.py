...
DEFAULT_ID_HASH_FUNCTION = lambda doc: "{:02x}".format(
    mmh3.hash128(str(doc.content), signed=False)
)
...
class Document:
    ...
    id_hash_function: Callable[["Document"], str] = field(
        default=DEFAULT_ID_HASH_FUNCTION, repr=False
    )
    ...
    def to_dict(self):
        return asdict(
            self,
            dict_factory=lambda x: {k: v for (k, v) in x if k != "id_hash_function"},
        )
    ...
    def from_dict(
        cls,
        dictionary,
        id_hash_function: Callable[["Document"], str] = DEFAULT_ID_HASH_FUNCTION,
    ):
