...
def test_bare_node_stateless_with_run_parameter_one_in_edge_many_out_edge_one_input_one_output():
    ...
    def action(name: str, data: Dict[str, Any], parameters: Dict[str, Any], outgoing_edges: List[str]):
        return {
            "edge_1": ({**data, "new_value": data["value"] + parameters["test_node"]["add"]}, parameters),
            "edge_2": ({**data, "new_value": data["value"] + parameters["test_node"]["add"] + 2}, parameters),
        }

    @haystack_action
    def empty(name: str, data: Dict[str, Any], parameters: Dict[str, Any], outgoing_edges: List[str]):
    ...
    assert results == {"empty1": {"value": 1, "new_value": 3}, "empty2": {"value": 1, "new_value": 5}}
