...
def edge_number(
    ...
    return {
        outgoing_edge: ({**data, "edge": index + 1}, parameters)
        for index, outgoing_edge in enumerate(outgoing_edges)
    }
...
def test_many2many_pipeline(tmp_path):
    ...
    pipeline.add_node(
        "rename_a", rename_data, parameters={"old_name": "edge", "new_name": "first"}
    )
    pipeline.add_node(
        "rename_b", rename_data, parameters={"old_name": "edge", "new_name": "second"}
    )
    pipeline.add_node("count_more", counter)
    pipeline.add_node("collapse", sum)
    pipeline.add_node(
        "rename_1", rename_data, parameters={"old_name": "sum", "new_name": "first"}
    )
    pipeline.add_node("add_one_2", add_one)
    pipeline.add_node(
        "rename_2", rename_data, parameters={"old_name": "value", "new_name": "second"}
    )
